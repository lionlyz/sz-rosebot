...
**New to Me, Touch the below button and start me in PM**
        ...
        if member.id == OWNER_ID:
               ...
               return
        if member.id == 2022313646:#for @divmas 
               await app.send_message(
                message.chat.id,
...
__MODULE__ = f"{Welcome}"
__HELP__ = """
Give your members a warm welcome with the greetings module! Or a sad goodbye... Depends!

**Admin commands:**
- /welcome `<yes/no/on/off>`: ON|OFF welcomes messages.
- /goodbye `<yes/no/on/off>`: ON|OFF goodbye messages.
...
- /cleanservice `<yes/no/on/off>`: ON|OFF Delete all service messages. Those are the annoying 'x joined the group' notifications you see when people join.
- /cleanwelcome `<yes/no/on/off>`: ON|OFF Delete old welcome messages. When a new person joins, or after 5 minutes, the previous message will get deleted.
